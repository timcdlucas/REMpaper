...
cols <- c( brewer.pal(9, 'Pastel1'), brewer.pal(10, 'Set3'), brewer.pal(9, 'Paired'))[-c(4,5,7)]
...
cellCol <- 'blue'
cellLwd <- 2.3
segments(2*pi, 0, 2*pi, 2.3*pi, col=cellCol, lwd=cellLwd)
segments(pi, 0, pi, 2.3*pi, col=cellCol, lwd=cellLwd)
segments(pi/2, 0, pi/2, 2.3*pi, col=cellCol, lwd=cellLwd)
segments(0, pi, 2.3*pi, pi, col=cellCol, lwd=cellLwd)
segments(0, 2*pi, pi, 2*pi, col=cellCol, lwd=cellLwd)        
...
pdf('~/Dropbox/PhD/Analysis/REM-chapter/imgs/equalRegions.pdf', family='Helvetica')
...
plot(c(0,0), c(0,0) , type='l', ylim=c(0,2.3*pi), xlim=c(0,2.3*pi),
      xlab= substitute(paste('Sensor Angle, ',theta)), ylab=substitute(paste('Call Angle, ',alpha)), xaxt='n', yaxt='n')
...
for(i in 1:length(polys)){
     polygon(polys[[i]][[1]], polys[[i]][[2]], col=cols[type[i]], border=gray(0.7), lwd=0.5)
}
...
plot(c(0,0), c(0,0) , type='l', ylim=c(0,2.3*pi), xlim=c(0,2.3*pi),
      xlab='Sensor Angle $\\theta$', ylab='Call Angle $\\alpha$', xaxt='n', yaxt='n')
for(i in 1:length(polys)){
